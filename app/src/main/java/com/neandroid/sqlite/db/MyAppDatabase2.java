...
public abstract class MyAppDatabase2 extends RoomDatabase {
     ...
     * 数据库名称
     */
    private static final String DATABASE_NAME = "my_database2.db";

    /**
     * 获取数据库实例-单例实现
     */
    private static MyAppDatabase2 INSTANCE;

    /**
     * 单例
     */
    public static MyAppDatabase2 getInstance(Context context) {
        if (INSTANCE == null) {
            synchronized (MyAppDatabase2.class) {
                if (INSTANCE == null) {
                    INSTANCE = Room.databaseBuilder(context.getApplicationContext(), MyAppDatabase2.class, DATABASE_NAME)
